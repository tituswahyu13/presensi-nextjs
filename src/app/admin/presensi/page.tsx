...
export default async function AdminPresensiPage({
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-[var(--text-primary)]">Rekap Presensi Harian</h1>
        <p className="text-[var(--text-muted)] mt-1">Kelola dan pantau kehadiran pegawai secara real-time.</p>
      </div>
            ...
            <tbody className="divide-y divide-gray-100 text-sm">
              {presensiData.length === 0 ? (
                ...
                presensiData.map((item) => (
                  <tr key={item.id} className="hover:bg-[var(--bg-card)]/50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="font-semibold text-[var(--text-primary)]">{item.pegawai?.nama || 'Unknown'}</div>
                      <div className="text-xs text-[var(--text-muted)]">NIK: {item.pegawai?.nik || '-'}</div>
                    </td>
                    <td className="px-6 py-4">
                      <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-cyan-900/50 text-cyan-400 border border-cyan-800/50">
                        {item.pegawai?.bagian || '-'}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4 text-center">
                      {item.foto_masuk ? (
                        <a href={`/${item.foto_masuk}`} target="_blank" rel="noopener noreferrer" className="inline-block p-1 border border-[var(--border)] rounded-lg hover:border-cyan-500 transition-colors">
                    ...
                    <td className="px-6 py-4 text-center">
                      {item.foto_keluar ? (
                        <a href={`/${item.foto_keluar}`} target="_blank" rel="noopener noreferrer" className="inline-block p-1 border border-[var(--border)] rounded-lg hover:border-cyan-500 transition-colors">
